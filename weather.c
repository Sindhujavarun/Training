// show the weather report of a given city.
...
void main()
{
    ...
    scanf("%s", cityName);
	sprintf(url, "wget -O weather.dat \"https://api.openweathermap.org/data/2.5/weather?q=%s&appid=7c13cfcd4e9bef72a7befcd048b5b005&units=metric\" --no-check-certificate", cityName);
    system(url);
